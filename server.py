...
app = FastAPI(
    title="Radiology Report Classification",
    version=APP_VERSION,
...
def _extract_patient_context(text: str) -> Dict[str, str]:
    patient_name = _extract_by_label(
        text, [r"patient\s*name", r"name"], max_len=100
    )
    patient_id = _extract_by_label(
        text, [r"patient\s*id", r"mrn", r"uhid", r"accession\s*no"], max_len=64
    )
    patient_dob = _extract_by_label(
        text, [r"dob", r"date\s*of\s*birth", r"birth\s*date"], max_len=32
    )
    physician = _extract_by_label(
        text, [r"referring\s*physician", r"physician", r"doctor"], max_len=100
    )
    hospital_name = _extract_by_label(
        text, [r"hospital", r"facility", r"institution", r"center", r"centre"], max_len=120
    )
...
def _parse_structured_sections(text: str) -> Dict[str, str]:
    section_pattern = re.compile(
        r"(?im)^(clinical indication|indication|examination technique|technique|method|findings|impression|conclusion|recommendations?)\s*[:\-]?\s*$"
    )
...
def load_models():
    try:
        ...
        with warnings.catch_warnings(record=True) as caught:
            # Catch ALL warnings generically - no dependency on InconsistentVersionWarning
...
async def analyze_findings(request: AnalysisRequest):
    ...
    try:
        ...
        resolved_patient_name = _merge_identity_value(
            request.patient_name, extracted_context.get("patient_name", ""), "N/A"
        )
        resolved_patient_dob = _merge_identity_value(
            request.patient_dob, extracted_context.get("patient_dob", ""), "N/A"
        )
        resolved_patient_id = _merge_identity_value(
            request.patient_id, extracted_context.get("patient_id", ""), "REF-UNKNOWN"
        )
        resolved_physician = _merge_identity_value(
            request.referring_physician, extracted_context.get("physician", ""), "N/A"
        )
        resolved_hospital_name = _merge_identity_value(
            request.hospital_name, extracted_context.get("hospital_name", ""), "CityCare Medical Center"
        )
        ...
        patient_thread_key = _build_patient_thread_key(
            resolved_patient_id, resolved_patient_name, resolved_patient_dob, findings_raw
        )
        patient_identity_source = _derive_identity_source(
            request.patient_name, request.patient_dob, request.patient_id, extracted_context
        )

        sections = _parse_structured_sections(findings_raw)

        indication = (
            sections.get("indication")
            or extract_section(findings_raw, "Clinical Indication")
            or "General radiological evaluation"
        )
        technique = (
            sections.get("technique")
            or extract_section(findings_raw, "Examination Technique")
            or "Standard radiology protocol"
        )
