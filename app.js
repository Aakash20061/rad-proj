...
const app = {
  config: {
    // Prefer environment-injected base, fall back to same-origin /api/v1
    apiBase:
      (window.__RADIUS_API_BASE__ && String(window.__RADIUS_API_BASE__)) ||
  ...
  loadPersistedState() {
    console.info("PHI persistence disabled. Running in volatile memory mode for compliance.");
    this.state.threads = [];
    this.state.activeThreadId = null;
  },

  persistState() {
    // Intentionally empty: We NO LONGER store PHI in localStorage
    // to comply with basic medical data security parameters. 
  },
